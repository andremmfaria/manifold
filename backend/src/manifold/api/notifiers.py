...
from manifold.schemas.common import DeletedResponse
from manifold.schemas.notification_deliveries import NotificationDeliveryListResponse
from manifold.schemas.notifiers import (
...
@router.get("", operation_id="listNotifiers", response_model=NotifierListResponse)
async def list_notifiers(
    ...
    total_result = await session.execute(
        select(func.count())
        .select_from(NotifierConfig.__table__)
...
@router.get("/{notifier_id}", operation_id="getNotifier", response_model=NotifierResponse)
...
async def update_notifier(
    ...
    async def _update() -> dict:
        ...
        if notifier is None:
            raise HTTPException(status_code=404, detail={"error": "not_found"})
        next_config = (
            dict(payload.config) if payload.config is not None else dict(notifier.config or {})
        )
...
@router.post(
    ...
    response_model=NotifierTestResponse,
)
async def test_notifier(
    ...
    result = await dispatcher.dispatch_test(notifier_id, owner_user_id)
    return NotifierTestResponse(ok=bool(result.get("delivered")))


@router.get(
    "/{notifier_id}/deliveries",
    operation_id="listNotifierDeliveries",
    response_model=NotifierDeliveryListResponse,
)
async def list_notifier_deliveries(
    ...
    async def _list() -> dict:
        total_result = await session.execute(
            ...
            .where(NotificationDelivery.__table__.c.notifier_id == parse_uuid(notifier_id))
        )
        total = total_result.scalar_one()
        result = await session.execute(
...
@delivery_router.get(
    ...
    operation_id="listLegacyNotificationDeliveries",
