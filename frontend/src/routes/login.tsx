    ...
    <main className="flex min-h-screen items-center justify-center bg-slate-50 p-6">
      <form onSubmit={onSubmit} className="w-full max-w-md rounded-xl bg-white p-8 shadow-sm">
        ...
        <label className="mb-4 block">
          ...
          <input className="w-full rounded border border-border p-3" placeholder="Email or username" value={username} onChange={(e) => setUsername(e.target.value)} />
        </label>
        <label className="mb-4 block">
          <span className="mb-2 block text-sm font-medium">Password</span>
          <input className="w-full rounded border border-border p-3" type="password" value={password} onChange={(e) => setPassword(e.target.value)} />
        </label>
