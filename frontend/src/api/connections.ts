...
export const connectionsApi = {
  ...
  async create(payload: ConnectionCreatePayload) {
    const response = await client.post<Connection>('/api/v1/connections', payload)
    return response.data
  },
  ...
  async update(connectionId: string, payload: ConnectionUpdatePayload) {
    const response = await client.patch<Connection>(`/api/v1/connections/${connectionId}`, payload)
    return response.data
  },
  ...
  async syncRuns(connectionId: string) {
    const response = await client.get<SyncRun[]>(`/api/v1/connections/${connectionId}/sync-runs`)
    return response.data
  },
